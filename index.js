...
client.on('ready', () => {
  ...
  let status = [`Bailo & Cat✨`,`${prefix}help`,`Love`];
  setInterval(()=>{
...
const owner = ["864554279276904498,980970172665962546"]///owner id
const supportServer = "https://discord.gg/8pUJgNpjvW"///server support
const help_info_color = "red"///color in help
...
message.lineReply(`يا مسكييين روح دور على شغل احسن لك
 `)
...
///line
client.on("message", message => {
  if (message.content == ("https://media.discordapp.net/attachments/1034883261916258327/1034932968088670319/4625701dd73af446.gif") || message.content === "https://media.discordapp.net/attachments/1034883261916258327/1034932968088670319/4625701dd73af446.gif") 
  {
...
message.lineReply(`
> \`-\` **My Link :https://discord.com/invite/Tdz8kg8hWx** 
...
client.on("message", async message => {
    ...
    if (command == "a") {
        let args = message.content.split("a")
...
//tax
let autotax = ['channel_id_here']; //change channel_id_here with your channel id

client.on("messageCreate", message => {
  if (messafe.channel.type == "dm" ||
  ...
  if (autotax.includes(message.channel.id)) {
      ...
      if (args.endWith("m")) args = args.replace(/m/gi,"") * 1000000;
      if (args.endWith("M")) args = args.replace(/m/gi,"") * 1000000;
      if (args.endWith("k")) args = args.replace(/m/gi,"") * 1000;
      if (args.endWith("K")) args = args.replace(/m/gi,"") * 1000;
      ...
      .addFields({
          name : "> **السعر بدون ضراءب**", value: `**\`${args}\`**`
      },
      {
          name : "> **السعر مع الضراءب**", value: `**\`${tax}\`**`
      },
      {
          name : "> **ضراءب الوسيط بدون نسبة**", value: `**\`${tax4}\`**`
      },
      {
          name : "> **ضراءب الوسيط مع نسبة***", value: `**\`${tax6}\`**`
      },
      {
          name : "> **نسبة الوسيط**", value: `**\`${tax9}\`**`
      },
      {
          name : "> **تحويل بدون ضراءب**", value: `**\`${tax12}\`**`
      })
...
client.on("message", async(black) => {
    ...
    if (black.content.startsWith("ping")) {
...
`)
  ...
  .setThumbnail(message.author.avatarURL({dynamic: true}))
        .setAuthor(message.author.username,message.author.avatarURL({dynaimc: true}))
           .setTimestamp()
       .setColor(color)
  if (message.channel.id != "1188244621839568926") return;
...
client.on("message", message => {
  ...
  .setImage(line)
       ...
       .setColor(color)
  if (message.channel.id != "") return;///channel id
...
client.on("message", message => {
  ...
  .setImage(line)
       ...
       .setColor(color)
  if (message.channel.id != "1188242151008649376") return;
...
client.on('message', async normal => {
  if (normal.content.startsWith(prefix + "m")){
...
client.on('message', async normal => {
  if (normal.content.startsWith(prefix + "unm")){
...
client.on("channelCreate", wanted => {
  if(wanted.parentID == "1188205726796099584") {
    setTimeout(() => {
...
.setTitle("> \`#\` **Prices for : boosts** ")
        ...
        .setDescription(`
        \`*1\` **__2boost 3 Month__ (__10m__)**  

        \`*2\` **__1boost 3 Month__ (__5m__)** 
...
//server banner
client.on('message', message => {
  if(message.content.startsWith('s-banner')) {
     message.lineReply(
      new Discord.MessageEmbed()
      .setThumbnail(message.guild.iconURL({ dynamic: true }))
      .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
...
> **${prefix}m  : ( __ لعمل ميوت لي شخص __ )**


> **${prefix}unm  : ( __ لفك الميوت __ )**


> **lock  : ( __ لقفل الاتشنل__ )**


> **unlock  : ( __ لفتح الاتشنل __ )**
...
> **hide  : ( __ لاخفاء الاتشنل__ )**


> **show  : ( __ لاظهار الاتشنل __ )**
...
.setImage(line)
    ...
    .setDescription(`
> **${prefix}a  : ( __لاظهار الافتار الخاص بيك__ )**


> **user  : ( __لاظهار معلومات عنك__ )**


> **${prefix}s  : ( __لاظهار معلومات عن السرفر__ )**


> **${prefix}tax  : ( __لاظهار الضريبه __ )**


> **s-banner  : ( __لاظهار بنر السرفر __ )**
 

> **ping  : ( __ لاظهار بنج البوت __ )**


> **p  : ( __لاظهار البروفايل الخاص بك __ )**
...
/*
///suggestions
let sug = ['1188242151008649376'];//channel sug
...
client.on("message", mor3eb => {
  let wprefix = prefix
    if (mor3eb.content.startsWith("set-welcome")) {
